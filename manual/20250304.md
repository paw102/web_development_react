...
그리고 VS code 역시 MS 에서 개발했기 때문에 지원이 많이 되는 이유로 TS를 선택함
...
- 그래서 첫 번째 ts 수업에서의 부분과 같이 타입 추론을 통해서 id: number 로 ts 상에서 결론이 났을 경우 student.id= '이번' 과 같은 방식으로 추론해서 어긋나느 자료형으로 재대입을 한 경우 ts 에서는 오류 메시지를 출력하게 됨.
